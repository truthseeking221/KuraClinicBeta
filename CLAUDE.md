...
- Before placing any container, choosing any background, or reaching for a shadow, read [the Kura surface direction](.codex/skills/design-kura-ui/references/kura-surface-direction.md) completely. Use a white page; group by proximity, alignment, spacing, and typography first; use flat gray trays only for groups that need a distinct functional region; use white tiles with one feather shadow; reserve colour for data, status, and the single primary action.
