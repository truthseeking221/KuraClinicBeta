...
## Kura Icon Illustration Style

Use this style for generated Kura launcher, empty-state, onboarding, and gentle guidance icons. The style must stay consistent with the current explorer-home icon set:

- `public/generated/kura-explore-demo-patient-v1.png`
- `public/generated/kura-explore-lab-catalog-v1.png`
- `public/generated/kura-explore-order-walkthrough-v1.png`
- source sheet: `public/generated/kura-explore-icon-illustrations-v2-flat.png`

### Style DNA

Generate clean, flat, stroke-filled medical product icons for Kura. The image should feel calm, clinical, premium, and friendly, with simple symbolic objects built from filled shapes plus visible stroke accents. Use no 3D, no isometric perspective, no glossy rendering, no photorealism, no heavy shadows, and no decorative background scene.

The illustration should read as a polished app icon illustration, not a generic line icon. Each object has a soft rounded silhouette, balanced negative space, and a tiny amount of depth from pale blue/white gradients only.

### Palette

Use only three Kura blues plus white. Keep saturation restrained and healthcare-clean.

```text
Deep blue stroke/detail: #175499
Primary blue fill:       #268CFF
Soft blue fill:          #D4E8FF
White/highlight:         #FFFFFF
```

Allowed tint behavior: use opacity or very soft blends of the three blues and white to create pale edges, but do not introduce purple, teal, green, amber, gray, black, or multicolor accents. White may be used for inner counters, highlights, soft center glow, and negative space.

### Shape Rules

- Flat front-facing or very shallow orthographic view only.
- Rounded geometry, softened terminals, and smooth corners.
- Stroke-filled style: the main form is filled, with simple stroke accents that define the symbol.
- Stroke weight should feel medium, about 5-8% of the icon object's narrow dimension.
- Avoid thin hairline details; details must survive at 64px display size.
- Use 1-3 primary objects maximum. No busy scenes.
- Keep the silhouette centered and readable before details are noticed.
- Prefer simple healthcare/workflow metaphors: chart, book, flask, shield, calendar, tube, note, check, clock, patient card.

### Depth Rules

- Depth is quiet: a soft white-to-blue gradient inside the shape or behind the central form.
- Use a very subtle pale halo or inner highlight only when it helps the object separate from the white UI.
- No cast shadow, no drop shadow, no bevel, no extrusion, no texture, no grain.
- Highlights should be broad and soft, never shiny or plastic.

### Canvas And Crop

- Generate a square icon asset at 550 x 550 px.
- Background should be white or near-white, with no frame.
- The object should occupy roughly 62-72% of the canvas width.
- Leave generous optical padding on all sides so it can sit inside Kura launcher tiles without feeling cramped.
- Keep the visual center slightly above mathematical center when the icon includes text-like rows or base elements.

### Prompt Template

```text
Create a single flat stroke-filled icon illustration for Kura: [ICON SUBJECT].

Style: clean healthcare SaaS product icon, calm clinical premium feel, rounded filled shapes with simple stroke accents, flat front-facing composition, no 3D, no isometric view, no photorealism.

Palette: use only #175499 deep blue, #268CFF primary blue, #D4E8FF soft blue, and #FFFFFF white. Add a very soft white-to-pale-blue gradient for subtle depth. No other colors.

Composition: centered square 550x550 icon, object fills about two-thirds of the canvas, generous white padding, clear silhouette, readable at small UI sizes, minimal details.

Finish: crisp edges, smooth rounded corners, soft internal highlight, no drop shadow, no texture, no background scene.
```

### Negative Prompt

```text
No 3D, no isometric angle, no realistic render, no glossy plastic, no heavy shadow,
no dark outline, no black, no gray palette, no purple, no green, no amber, no red,
no gradients except soft white-to-blue depth, no tiny labels, no busy dashboard,
no background scene, no badge frame, no mascot, no stock illustration style.
```

### QA Checklist

- Uses exactly the three Kura blues plus white.
- Still reads clearly at 64px.
- Feels like the three explorer-home examples when viewed side by side.
- Has filled surfaces and stroke accents, not only line art.
- Has a soft white gradient or highlight, but no 3D lighting.
- Has enough white padding to sit inside an 86px Kura launcher tile.
- Does not introduce a new visual metaphor style, angle, palette, or texture.
