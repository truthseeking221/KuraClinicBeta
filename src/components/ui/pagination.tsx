...
export const Pagination = forwardRef<HTMLElement, PaginationProps>(function Pagination(
  ...
  const t = useT();
  return <nav ref={ref} aria-label={label ?? t('Pagination')} className={joinClasses(styles.root, className)} {...props} />;
});
...
export const PaginationPrevious = forwardRef<HTMLAnchorElement, PaginationPreviousProps>(function PaginationPrevious(
  ...
  const t = useT();
  return <PaginationLink ref={ref} aria-label={t('Go to previous page')} {...props}><ChevronLeftIcon aria-hidden size={16} /><span className={styles.directionLabel}>{label ?? t('Previous')}</span></PaginationLink>;
});
...
export const PaginationNext = forwardRef<HTMLAnchorElement, PaginationNextProps>(function PaginationNext(
  ...
  const t = useT();
  return <PaginationLink ref={ref} aria-label={t('Go to next page')} {...props}><span className={styles.directionLabel}>{label ?? t('Next')}</span><ChevronRightIcon aria-hidden size={16} /></PaginationLink>;
});
