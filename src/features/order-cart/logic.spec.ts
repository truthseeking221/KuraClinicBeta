...
describe('unified order cart role contract', () => {
  ...
  it('allows a doctor to edit only an unlocked draft', () => {
    ...
    expect(canEditOrderCart(cart, doctorWorkflow({ paymentLocked: true }))).toBe(false);
...
describe('collection & payment decisions', () => {
  it('requires who collects, payment, and a site when Kura collects', () => {
    ...
    expect(decisionsComplete({ collectBy: 'kura', payment: 'pay-now' })).toBe(false);
    expect(
      decisionsComplete({ collectBy: 'kura', drawSite: 'patient-home', payment: 'pay-now' }),
    ).toBe(true);
    expect(decisionsComplete({ collectBy: 'self', payment: 'pay-now' })).toBe(true);
  });
  ...
  it('summarizes the decisions in Figma vocabulary per stage', () => {
    ...
    ).toEqual({ title: 'Kura collection · Patient Home', detail: 'Patient pays later at Kura' });
    expect(doctorDecisionSummary(DOCTOR_CODE_SENT)?.detail).toBe('Code sent');
    expect(
      doctorDecisionSummary(doctorWorkflow({ decisions: { collectBy: 'self', payment: 'pay-now' } })),
    ).toEqual({ title: 'Clinic draw · you collect', detail: 'Patient pays you now' });
  });
});

describe('earnings math', () => {
  it('derives the Figma board earn amount from basis points', () => {
    expect(earnMinorFromBp('16600', 1500)).toBe('2490');
  });

  it('floors to whole minor units and clamps bp into contract range', () => {
    expect(earnMinorFromBp('999', 1500)).toBe('149');
    expect(earnMinorFromBp('16600', 20000)).toBe('16600');
    expect(earnMinorFromBp('16600', -5)).toBe('0');
  });
});

describe('tube preparation', () => {
  it('tracks scan progress toward completion', () => {
    expect(tubeProgress(DEMO_TUBES)).toEqual({ scanned: 0, total: 2, complete: false });
    expect(
...
describe('primary action state machine', () => {
  it('walks the doctor Kura-collection path', () => {
    expect(getOrderCartPrimaryAction(EMPTY_CART, doctorWorkflow())).toMatchObject({
      label: 'Send booking code',
      disabled: true,
    });
    expect(getOrderCartPrimaryAction(FULL_CART, doctorWorkflow())).toMatchObject({
      disabled: true,
  ...
  it('walks the self-draw path through tube prep', () => {
    ...
    expect(
      getOrderCartPrimaryAction(FULL_CART, doctorWorkflow({ ...allScanned, stage: 'confirmed' })),
  ...
  it('yields no cart action while reception is still composing the order', () => {
    expect(
      getOrderCartPrimaryAction(FULL_CART, receptionistWorkflow({ stage: 'order-review' })),
    ).toBeNull();
  });

  it('walks the receptionist tender path', () => {
    expect(getOrderCartPrimaryAction(FULL_CART, receptionistWorkflow())).toMatchObject({
      label: 'Confirm payment & check in',
      disabled: true,
      disabledReason: 'Choose a payment method first.',
    });

    const cashChosen = receptionistWorkflow({ method: 'cash', panel: 'summary' });
    ...
    const deferred = receptionistWorkflow({ method: 'pay-later', panel: 'summary' });
    ...
    expect(
      getOrderCartPrimaryAction(
        FULL_CART,
        receptionistWorkflow({ stage: 'checked-in', method: 'cash', attested: true }),
      ),
