...
export function canEditOrderCart(
  ...
  if (workflow.role === 'doctor') {
    return workflow.authority !== 'read-only' && workflow.stage === 'draft' && !workflow.paymentLocked;
  }
...
export function doctorDecisionSummary(workflow: DoctorOrderCartWorkflow): {
  ...
  const site = decisions.drawSite === 'patient-home' ? 'Patient Home' : 'Kura PSC';
  return {
...
/* ── Earnings (doctor commission economics) ── */

/** Earn = subtotal × bp / 10000, floored to a whole minor unit. */
export function earnMinorFromBp(subtotalMinor: string, commissionBp: number): string {
  const subtotal = BigInt(subtotalMinor);
  const bp = BigInt(Math.max(0, Math.min(10000, Math.round(commissionBp))));
  return ((subtotal * bp) / BigInt(10000)).toString();
}
...
export function tubeProgress(tubes: readonly OrderCartTube[]): {
  ...
  return { scanned, total: tubes.length, complete: tubes.length > 0 && scanned === tubes.length };
}
...
function receptionistAction(
  ...
  if (!workflow.method) {
    return { label, disabled: true, disabledReason: 'Choose a payment method first.' };
  }
...
export function getOrderCartPrimaryAction(
  ...
  if (workflow.access !== 'allowed') {
    const label = workflow.role === 'doctor' ? 'Send booking code' : 'Confirm payment & check in';
    return {
      label,
      disabled: true,
      disabledReason:
