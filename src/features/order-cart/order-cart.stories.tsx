...
function Playground({
  ...
  const repricedCart = (items: OrderCartItem[]): OrderCartData => {
    ...
    return { ...base, ...cartWith(items, { id: cart.id, patient: cart.patient }) };
  };
  ...
  const advance = () => {
    ...
    if (workflow.role === 'doctor') {
      setWorkflow((current) => {
        ...
        if (doctor.stage === 'code-sent' || doctor.stage === 'collected') return doctor;
        if (doctor.decisions.collectBy === 'self') {
          return { ...doctor, stage: 'tubes', tubes: doctor.tubes ?? DEMO_TUBES };
        }
        return { ...doctor, stage: 'code-sent', panel: 'summary', paymentLocked: true };
      });
      return;
    }
    setWorkflow((current) => {
      ...
      return {
        ...
        payment:
          reception.method === 'pay-later'
            ...
            : {
                status: 'paid',
                label: reception.method === 'khqr' ? 'KHQR confirmed' : 'Cash collected',
                receiptId: 'R-58213',
              },
      };
    });
  ...
  return (
    <div className={mobile ? styles.mobileCanvas : styles.canvas}>
      <OrderCart
        ...
        onAcceptReprice={() => {
          ...
          setCart((current) => {
            ...
            const newPrices = new Map(
              current.pricing.repricedLines.map((line) => [line.itemId, line.newPriceMinor]),
            );
            return {
              ...current,
        ...
        onAddFirst={() => {
          args.onAddFirst?.();
          setCart(repricedCart([FIGMA_TESTS[0]]));
        }}
        ...
        onClear={() => {
          args.onClear?.();
          setCart(repricedCart(cart.items.filter((item) => item.state === 'locked')));
        }}
        ...
        onRetryPricing={() => {
          ...
          setCart((current) =>
            current.pricing.state === 'error'
              ? { ...current, ...cartWith(current.items, { id: current.id, patient: current.patient }) }
              : current,
          );
        }}
...
export const DoctorEmpty: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('heading', { name: 'Order Cart' })).toBeVisible();
    await expect(canvas.getByText('Nothing here yet')).toBeVisible();
    await userEvent.click(canvas.getByRole('button', { name: 'Add first test' }));
    await expect(meta.args.onAddFirst).toHaveBeenCalled();
    await expect(canvas.getByRole('heading', { name: 'Selected tests' })).toBeVisible();
...
export const DoctorNotConfigured: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('button', { name: 'Send booking code' })).toBeDisabled();
    ...
    await expect(canvas.getByRole('radio', { name: 'Kura will draw the blood' })).toBeVisible();
  },
};
...
export const DoctorDecisionExpanded: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('radio', { name: 'Patient will pay you now' })).toBeChecked();
    ...
    await expect(canvas.getByRole('button', { name: 'Edit collection and payment' })).toBeVisible();
  },
};
...
export const DoctorEditDecisions: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: 'Edit collection and payment' }));
    await expect(meta.args.onPanelChange).toHaveBeenCalledWith('expanded');
    await expect(canvas.getByRole('radio', { name: 'Kura will draw the blood' })).toHaveFocus();
    const payNow = canvas.getByRole('radio', { name: 'Patient will pay you now' });
    await expect(payNow).toBeEnabled();
    await userEvent.click(payNow);
    await userEvent.click(canvas.getByRole('button', { name: 'Done' }));

    await expect(canvas.getByText('Patient pays you now')).toBeVisible();
    await expect(canvas.getByRole('button', { name: 'Edit collection and payment' })).toHaveFocus();
...
export const DoctorPayNowNeedsAttestation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('button', { name: 'Send booking code' })).toBeEnabled();
  },
};
...
export const DoctorCodeSent: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: 'View collection and payment' }));
    await expect(canvas.getByRole('radio', { name: 'Kura will draw the blood' })).toBeDisabled();
    await expect(
...
export const DoctorExplorerBlocked: Story = {
  ...
  args: {
    workflow: doctorWorkflow({
      authority: 'explorer',
      panel: 'summary',
      decisions: { collectBy: 'kura', drawSite: 'kura-psc', payment: 'pay-later-kura' },
      earnings: undefined,
    }),
  },
  play: async ({ canvasElement }) => {
...
export const ReceptionPaymentDue: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByRole('complementary', { name: 'Receptionist order cart' })).toBeVisible();
    ...
    await expect(meta.args.onPanelChange).toHaveBeenCalledWith('expanded');
    await expect(canvas.getByRole('radio', { name: 'Cash at the desk' })).toBeVisible();
  },
};
...
export const ReceptionMethodChoice: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByRole('radio', { name: 'Cash at the desk' })).toBeChecked();
    ...
    await expect(canvas.getByRole('radio', { name: 'KHQR transfer' })).toBeChecked();
    await userEvent.click(canvas.getByRole('radio', { name: 'Pay later at Kura' }));
    await expect(canvas.getByRole('radio', { name: 'Pay later at Kura' })).toBeChecked();
...
export const ReceptionAttestationGate: Story = {
  ...
  args: { workflow: receptionistWorkflow({ method: 'cash', panel: 'summary' }) },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    await userEvent.click(canvas.getByRole('checkbox'));
    await expect(meta.args.onAttestChange).toHaveBeenCalledWith(true);
    await expect(canvas.getByRole('button', { name: 'Confirm payment & check in' })).toBeEnabled();
  },
};

export const ReceptionPayLater: Story = {
  name: 'Reception · pay later at Kura',
  args: { workflow: receptionistWorkflow({ method: 'pay-later', panel: 'summary' }) },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByText('Payment · Pay later at Kura')).toBeVisible();
    await expect(canvas.queryByRole('checkbox')).not.toBeInTheDocument();
    await expect(canvas.getByRole('button', { name: 'Check in & confirm order' })).toBeEnabled();
  },
};
...
export const ReceptionPaidLocked: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('radio', { name: 'Cash at the desk' })).toBeDisabled();
    await expect(
      canvas.getByText('Locked after payment. Changes route through void or supplemental.'),
    ).toBeVisible();
  },
};
...
export const ReceptionBlockedPrescriber: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: 'Choose prescriber' }));
...
export const ReceptionSupplementalAfterPaidEdit: Story = {
  ...
  args: {
    cart: cartWith(
      [
        ...FIGMA_TESTS.slice(0, 3).map((item) => ({ ...item, state: 'locked' as const })),
...
export const PricingLoading: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const edit = canvas.getByRole('button', { name: 'Edit collection and payment' });
...
export const PricingError: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: 'Retry pricing' }));
  ...
  args: {
    cart: cartWith(FIGMA_TESTS, {
      pricing: {
        ...
        repricedLines: [
          { itemId: 'ferritin', name: 'Ferritin', oldPriceMinor: '1400', newPriceMinor: '1600' },
        ],
      },
    }),
    workflow: DOCTOR_PAY_LATER,
...
export const HierarchicalRelations: Story = {
    ...
    cart: cartWith([
      {
        ...
        children: [
          ...
          { id: 'cmp-creatinine', name: 'Creatinine', relation: 'panel_channel' },
      ...
      },
      {
        ...
        currencyCode: 'USD',
        quantity: 1,
        children: [{ id: 'egfr-crea', name: 'Creatinine', relation: 'derived_input' }],
      },
    ]),
    workflow: DOCTOR_NOT_CONFIGURED,
  },
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByRole('button', { name: 'Remove Glucose' })).not.toBeInTheDocument();
    await expect(canvas.getByRole('button', { name: 'Remove CMP (metabolic panel)' })).toBeVisible();
  },
};
      ...
      [
        {
          ...
          quantity: 1,
          children: [
            { id: 'cmp-glucose', name: 'Glucose', relation: 'panel_channel', creditMinor: '400' },
          ],
        },
        {
          id: 'glucose-f',
...
export const EngineSuggestions: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getAllByRole('button', { name: 'Keep as is' })).toHaveLength(1);
